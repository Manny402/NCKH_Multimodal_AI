...
\subsection{Field Validation Results}
During the actual acceptance testing conducted by the client, the system processed real-world inputs characterized by distorted handwriting, inconsistent document templates, multi-accented Vietnamese speech, and the prevalence of irregular product naming. Under these challenging operational conditions, the system achieved promising outcomes with an accuracy rate of over 75\%. The research team remains on-track to further optimize the pipeline, with ongoing refinements projected to increase the accuracy to over 85\%.

\subsection{Cost Efficiency Analysis}
Economic viability was estimated based on API token consumption during inference. Calculations assume an average processing volume of 1,200 input tokens and 150 output tokens per document. As reported in Table~\ref{tab:efficiency}, the total operational cost is estimated at \$13.50 per 1,000 processed documents\footnote{Cost estimation follows standard industry practice for applied LLM systems: average empirical token counts multiplied by publicly published cloud API pricing rates (e.g., Google Cloud Vision and Gemini API tier pricing as of current evaluation). Local inference components, such as ASR, incur zero API costs.}. The majority of this cost ($\approx$\$12.00) is attributed to the LLM API during the extraction and taxonomy prediction phases. 

To contextualize these savings, deploying a flagship end-to-end multimodal model (e.g., Gemini 2.5 Pro) to directly process the same raw visual and acoustic inputs would cost approximately \$30.00 per 1,000 documents. By decoupling the perceptual tasks (OCR/ASR) to specialized or local modules and restricting the LLM exclusively to text-based semantic reasoning, the proposed modular architecture achieves a \textbf{55\% cost reduction}. Given the substantial reduction in manual data entry requirements, this optimized cost profile indicates highly practical feasibility for scalable enterprise deployment.
...
To illustrate the operational impact, processing 1,000 B2B orders manually typically requires approximately 50 labor hours (assuming an average of 3 minutes per complex enterprise order). In contrast, the proposed system processes 1,000 continuous documents in less than 21 minutes of machine time (at $\sim$1.25 seconds per document). When factoring in the 12.5\% human-in-the-loop intervention rate required for ambiguous cases, the residual manual workload is proportionally minimized. This represents an \textbf{87.5\% reduction in the total human-equivalent workload}, dramatically accelerating the order-to-cash cycle while maintaining high data integrity.
